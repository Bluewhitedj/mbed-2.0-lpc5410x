...
#define STAT_MST_PENDING 0
#define STAT_MSTSTATE    1
#define STAT_RCV_RDY     (0x1)
#define STAT_TXM_RDY    (0x2)
#define STAT_NACK_ADDR   (0x3)
#define STAT_NACK_DAT    (0x4)

#define CFG_MSTEN        (0)
#define CFG_SLVEN        (1)


#define MSTCTL_MSTCONTINUE (0)
#define MSTCTL_MSTSTART    (1)
#define MSTCTL_MSTSTOP     (2)

#define SLVCTL_SLVCONTINUE  (0)
#define SLVCTL_SLVNACK      (1)
...
static int i2c_wait_SI(i2c_t *obj) {
    int timeout = 0;
    while (!(obj->i2c->STAT & (1 << STAT_MST_PENDING))) {
        timeout++;
...
static inline void i2c_interface_enable(i2c_t *obj) {
    obj->i2c->CFG |= (1 << CFG_MSTEN);
}

static inline void i2c_power_enable(i2c_t *obj) {
    LPC_ASYNC_SYSCON->ASYNCAPBCLKCTRLSET = (0x1 << (5 + obj->index));
    LPC_ASYNC_SYSCON->ASYNCPRESETCTRLSET = (0x1 << (5 + obj->index));
    LPC_ASYNC_SYSCON->ASYNCPRESETCTRLCLR = (0x1 << (5 + obj->index));
}
...
void i2c_init(i2c_t *obj, PinName sda, PinName scl) {
	...
	obj->i2c = (LPC_I2C_T *)i2c;
    switch (i2c)
   	{
      case I2C_0: obj->index = 0; break;
	  case I2C_1: obj->index = 1; break;
	  case I2C_2: obj->index = 2; break;
	}
    // enable power
...
inline int i2c_start(i2c_t *obj) {
	...
	//wait here until master is pending
    while(!(obj->i2c->STAT & (1 << STAT_MST_PENDING)));
	
    if (repeated_start) {
        obj->i2c->MSTCTL = (1 << MSTCTL_MSTSTART) | (1 << MSTCTL_MSTCONTINUE);
        repeated_start = 0;
    } else {
        obj->i2c->MSTCTL = (1 << MSTCTL_MSTSTART);
    }
    return status;
}
...
inline int i2c_stop(i2c_t *obj) {
    ...
    obj->i2c->MSTCTL = (1 << MSTCTL_MSTSTOP) | (1 << MSTCTL_MSTCONTINUE); // STP bit and Continue bit. Sends NAK to complete previous RD
    
    //Spin until master pending (b0 == 1)and Status is Idle (b3..b1 == 000)
    while ((obj->i2c->STAT & ((7 << STAT_MSTSTATE) | (1 << STAT_MST_PENDING))) != ((0 << STAT_MSTSTATE) | (1 << STAT_MST_PENDING))) { 
        timeout ++;
        if (timeout > 100000) return 1;
    }
...
static inline int i2c_do_write(i2c_t *obj, int value, uint8_t addr) {
    ...
    if (!addr)
        obj->i2c->MSTCTL = (1 << MSTCTL_MSTCONTINUE);
...
void i2c_frequency(i2c_t *obj, int hz) {
    ...
    uint32_t PCLK = ASYNC_CLOCK_FREQ;
	uint8_t sclH, sclL;
    uint32_t scl;
	
    //set clock divider as 4
    obj->i2c->CLKDIV = 4 - 1;

	//calculate scl high and scl low
	scl = PCLK / 4 / hz;
	sclH = scl >> 1;
	sclL = scl - sclH;
	if(sclH < 2)
    ...
    obj->i2c->MSTTIME = (((sclH - 2) & 0x7) << 4) | ((sclL - 2) & 0x7);
...
int i2c_read(i2c_t *obj, int address, char *data, int length, int stop) {
    ...
    for (count = 0; count < (length-1); count++) {
      ...
      i2c_wait_SI(obj);
      status = i2c_status(obj);
      if (status != STAT_RCV_RDY) { // RX RDY
        i2c_stop(obj);
        return count;
      }
      data[count] = I2C_DAT(obj) & 0xFF; // Store read byte

      obj->i2c->MSTCTL = (1 << MSTCTL_MSTCONTINUE); // Send ACK and Continue to read
    }
...
int i2c_write(i2c_t *obj, int address, const char *data, int length, int stop) {
    ...
    I2C_DAT(obj) = (address << 1) & 0xFE;   
    i2c_start(obj);
    
    // Wait for completion of STA and Sending of SlaveAddress+/WR
    i2c_wait_SI(obj);
    status = i2c_status(obj);    
    if (status == STAT_NACK_ADDR) { // NAK SlaveAddress
        i2c_stop(obj);
        return I2C_ERROR_NO_SLAVE;
    }
    ...
    for (i=0; i<length; i++) {
        ...
        if (status != STAT_TXM_RDY) { // TX RDY. Handles a Slave NAK on datawrite
            i2c_stop(obj);
            return i;
        }
    }
...
void i2c_slave_mode(i2c_t *obj, int enable_slave) {
  ...
  if (enable_slave) {
    obj->i2c->CFG |=  (1 << CFG_SLVEN); //Enable Slave mode
  }
  else {
    obj->i2c->CFG &= ~(1 << CFG_SLVEN); //Disable Slave mode
  } 
}
...
int i2c_slave_receive(i2c_t *obj) {
  ...
  switch(I2C_SLVSTAT(obj)) {
    case 0x0: // Slave address plus R/W received 
              ...
              obj->i2c->SLVCTL = (1 << SLVCTL_SLVCONTINUE); 
...
//};
  ...
  if (last) {
    obj->i2c->SLVCTL = (1 << SLVCTL_SLVNACK);  // Send NACK on received data and Continue 
  }    
  else {  
    obj->i2c->SLVCTL = (1 << SLVCTL_SLVCONTINUE);  // Send ACK on data and Continue to read                
  }
...
//  if (i2c_wait_slave_SI(obj) != 0) {return -2;} // Wait with timeout
  ...
  switch(I2C_SLVSTAT(obj)) {
    ...
    case 0x2: // Slave transmit. Data can be transmitted (Slave Transmitter mode).
              ...
              obj->i2c->SLVCTL = (1 << SLVCTL_SLVCONTINUE);   // Continue to send
...
void i2c_slave_address(i2c_t *obj, int idx, uint32_t address, uint32_t mask) {
  obj->i2c->SLVADR[0]   = ((address << 1) & 0xFE); // Store address in address 0 register
  obj->i2c->SLVADR[1]   = (0x00    & 0xFE); // Store general call write address in address 1 register
  obj->i2c->SLVADR[2]   = (0x01);           // Disable address 2 register
  obj->i2c->SLVADR[3]   = (0x01);           // Disable address 3 register
