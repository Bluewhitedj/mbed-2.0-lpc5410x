...
void us_ticker_init(void) {
    ...
    LPC_SYSCON->AHBCLKCTRL[1] |= (1 << 1);
    
    // Clear peripheral reset the RIT
    LPC_SYSCON->PRESETCTRL[1] |= (1 << 1);
    LPC_SYSCON->PRESETCTRL[1] &= ~(1 << 1);
...
void us_ticker_set_interrupt(timestamp_t timestamp) {
    ...
    LPC_RITIMER->COMPVAL = (temp & 0xFFFFFFFFL);
    LPC_RITIMER->COMPVAL_H = ((temp >> 32)& 0x0000FFFFL);
}
