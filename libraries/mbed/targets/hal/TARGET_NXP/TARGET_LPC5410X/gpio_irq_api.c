...
int gpio_irq_init(gpio_irq_t *obj, PinName pin, gpio_irq_handler handler, uint32_t id) {
    ...
    /* Enable AHB clock to the PINT, GPIO0/1, IOCON and MUX domain. */
    LPC_SYSCON->AHBCLKCTRL[0] |= ((1 << 18) | (0x1D << 11));
    ...
    if(obj->ch < 4)
   	{
	  LPC_SYSCON->STARTERP[0] |= (1 << (obj->ch + 5));
	}
	else
	{
      LPC_SYSCON->STARTERP[1] |= (1 << ((obj->ch-4) + 1));
	}
    //set the pin as input
    LPC_GPIO->DIR[pin >> 5]  &= ~(1 << (pin & 0x1F));
	...
	if(obj->ch < 4)
	{
	  NVIC_SetVector((IRQn_Type)(PIN_INT0_IRQn + obj->ch), (uint32_t)channels_irq);
	  NVIC_EnableIRQ((IRQn_Type)(PIN_INT0_IRQn + obj->ch));
	}
    else
   	{
      NVIC_SetVector((IRQn_Type)(PIN_INT4_IRQn + obj->ch - 4), (uint32_t)channels_irq);
	  NVIC_EnableIRQ((IRQn_Type)(PIN_INT4_IRQn + obj->ch - 4));
	}
...
void gpio_irq_free(gpio_irq_t *obj) {
    ...
    if(obj->ch < 4)
   	{
	  LPC_SYSCON->STARTERCLR[0] = (1 << (obj->ch + 5));
	}
	else
	{
      LPC_SYSCON->STARTERCLR[1] = (1 << ((obj->ch-4) + 1));
	}	
}
...
void gpio_irq_disable(gpio_irq_t *obj) {
  ...
  {
    NVIC_DisableIRQ((IRQn_Type)(PIN_INT4_IRQn + obj->ch - 4));
  }
}
